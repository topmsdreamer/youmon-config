...
import { FlatList, StyleSheet, Text, View, Switch, Alert, TouchableOpacity, SafeAreaView, Dimensions } from 'react-native';
import { Camera } from 'expo-camera';
import * as _ from 'lodash';
import  PrimaryButton  from "../components/DefaultButton";
...
export const UploadPhoto = (props:any) => {
  ...
  if (hasPermission === false) {
    return <View style={{ flex: 1, backgroundColor: '#fff' }}><Text>Ingen tillgång till kameran, lägg till den här appen i dina kamerainställningar</Text>
      <PrimaryButton onPress={() => cancelSnap()} text="Stäng"></PrimaryButton>
    </View>;
  }

  const screenWidth = Dimensions.get('screen').width < Dimensions.get('screen').height ? Dimensions.get('screen').width : Dimensions.get('screen').height;
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', backgroundColor: '#000' }}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View>
          {!isPreview && <View style={{ alignSelf: 'flex-end', margin: 10 }}>
            <TouchableOpacity
              onPress={() => {
                setType(
        ...
        <View style={{ flex: 1, justifyContent: 'flex-end', marginHorizontal: 10, marginBottom:5 }}>
          {!isPreview && <>
            <TouchableOpacity onPress={onSnap}><Ionicons style={{ alignSelf: 'center', marginBottom: 20 }} name="radio-button-on" size={120} color="#fff" /></TouchableOpacity>
          ...
          {isPreview && <>
            ...
            <PrimaryButton style={{marginBottom:10}} color={'secondary'} onPress={cancelPreview} text='New photo'></PrimaryButton>
          </>}
        </View>
      </SafeAreaView>
    </View>
