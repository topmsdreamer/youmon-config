...
const ValueBox = (props: any) => {
  ...
  const date = currentX
    ? moment(currentX).format("ddd MMM D YYYY HH:mm:ss")
    : lastX
    ? moment(lastX).format("ddd MMM D YYYY HH:mm:ss")
    : moment().format("ddd MMM D YYYY HH:mm:ss");
  const value = currentY ? currentY.toFixed(2) : lastY || "";
  ...
  return (
    <G x={10} y={40}>
      ...
      <SvgText y={25} fontSize={20} fontWeight="bold" fill="rgba(0, 0, 0, 1)">
        {name}
      </SvgText>
      <SvgText
        y={50}
...
class CustomFlyout extends React.Component<any> {
  componentDidMount() {
    ...
    if (onChange) {
      const data = { ...value, currentY: datum.y, currentX: datum.x };
      onChange(name, data);
    }
  }
  ...
  componentWillUnmount() {
    const { name, datum, value, onChange } = this.props;
    if (onChange) {
      onChange(name, { ...value, currentY: undefined, currentX: undefined });
    }
  }
    ...
    const g = (
      <G x={x}>
        <Line
          ...
          strokeDasharray={[6, 3]}
        />
        <Circle cy={y} r={5} stroke="#fff" strokeWidth={2} fill="#bf2c37" />
      </G>
    );
    return g;
  }
...
> {
  ...
  _getTickFormat = (value: any, index: number, ticks: any) => {
    ...
    const selectedStartDate =
      data.length > 0
        ? moment(data[0].x).startOf("day")
        : moment().add("-1", "day").startOf("day");
    const selectedEndDate =
      data.length > 0
        ? moment(data[data.length - 1].x).endOf("day")
        : moment().endOf("day");
  ...
  render() {
    ...
    const minDomain =
      options?.min !== undefined ? { minDomain: { y: options.min } } : {};
    const maxDomain =
      options?.max !== undefined ? { maxDomain: { y: options.max } } : {};
        ...
        {data && data.length > 0 && (
          ...
          >
            ...
            <VictoryAxis
              ...
              scale={{ y: "time" }}
              tickFormat={(tick, index, ticks) =>
                this._getTickFormat(tick, index, ticks)
              }
              tickLabelComponent={
                <VictoryLabel
            ...
            <VictoryLine
              style={{
                data: { stroke: "#192a46", strokeWidth: 2, opacity: 0.8 },
              }}
              data={data}
            />
          </VictoryChart>
